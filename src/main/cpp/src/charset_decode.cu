...
// GBK -> Unicode lookup table, materialized in device global memory at link
// time. Generated by src/main/cpp/scripts/GenerateGbkTable.java using Java's
// GBK CharsetDecoder. To regenerate:
//   javac GenerateGbkTable.java && java GenerateGbkTable > gbk_to_unicode_table.inc
//
// Putting the table in `__device__ const` storage means there is no runtime
// initialization path (no std::call_once, no RMM allocation, no
// stream.synchronize), so a GBK decode kernel cannot stall or leak on a
...
static_assert(sizeof(gbk_to_unicode_device) / sizeof(gbk_to_unicode_device[0]) == GBK_TABLE_SIZE,
              "GBK lookup table size must match GBK_TABLE_SIZE");
